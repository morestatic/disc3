...
func (opts *MakeArtistsReleasesXmlArchiveOptions) Execute(args []string) error {
	fmt.Printf("%#v\n", *opts)
	fmt.Println(args)
	...
	archiveDecoder, err := archives.NewDiscogsFileDecoder(archives.Releases, opts.DropId, opts.DropPath)
	...
	w, err := archives.NewDiscogsFileEncoder(archives.Releases, opts.OutputName)
	if err != nil {
	...
	err = commands.GenerateReleasesXml(opts.LibStagingPath, archiveDecoder, indexer, f, w, args)
	return err
}
